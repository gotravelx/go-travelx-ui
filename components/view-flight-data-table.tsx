...
import { ChevronDown, ChevronRight, Plane, Calendar, Copy, ExternalLink } from "lucide-react"
...
export default function ViewFlightDatTable({
  ...
  const columnDescriptions = {
    ...
    "Transaction ID": "Unique transaction hash (likely from a blockchain ledger)",
    ...
    "Flight Status Cd": "Abbreviated flight status (OUT = Departed, IN = Arrived, NDPT = Not Departed)",
        ...
        <Table data-testid="flight-table">
          <TableHeader>
            <TableRow className="bg-muted/50">
              ...
              <TableHead className="whitespace-nowrap">{renderTableHeaderWithTooltip("From")}</TableHead>
              <TableHead className="whitespace-nowrap">{renderTableHeaderWithTooltip("To")}</TableHead>
              <TableHead className="hidden md:table-cell">
                {renderTableHeaderWithTooltip("Departure DateTime")}
              </TableHead>

              <TableHead>{renderTableHeaderWithTooltip("Departure State")}</TableHead>

              <TableHead className="hidden md:table-cell">{renderTableHeaderWithTooltip("Arrival DateTime")}</TableHead>

              <TableHead>{renderTableHeaderWithTooltip("Arrival State")}</TableHead>
              <TableHead>{renderTableHeaderWithTooltip("Flight Status")}</TableHead>
              ...
              paginatedFlights.map((flight) => (
                <>
                  ...
                  >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <div className="flex items-center gap-2">
                                <span className="font-mono text-xs">{formatTxHash(flight.blockchainTxHash)}</span>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="h-6 w-6"
                    ...
                    <TableCell>
                      <div className="flex gap-1 items-center">
                        <Badge className="py-2 ">{mapArrivalStateCodeToText(flight?.departureState || "")}</Badge>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Button variant="outline" size="sm" onClick={() => openFlightDetails(flight)}>
                        Details
                      </Button>
                    </TableCell>
                      ...
                      <TableCell colSpan={10} className="bg-muted/20 p-0">
                        <div className="p-4">
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                            <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 text-sm gap-2">
                                ...
                                <div>{flight.scheduledDepartureUTCDateTime || "TBD"}</div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 text-sm gap-2">
                                ...
                                <div className="text-muted-foreground">Scheduled:</div>
                                <div>{flight?.scheduledArrivalUTCDateTime || "TBD"}</div>
                                <div className="text-muted-foreground">Status:</div>
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 text-sm gap-2">
                                ...
                                <div className="text-muted-foreground">Status:</div>
                                <div>
                                  <Badge variant="outline" className={`${getStatusBadgeColor(flight)}`}>
                          ...
                          <div className="flex justify-end mt-4">
                            <Button onClick={() => openFlightDetails(flight)} className="gradient-border">
                              View Full Details
                            </Button>
                          </div>
                        </div>
      ...
      {totalPages > 0 && (
        <div className="flex justify-between items-center mt-4">
          <div className="text-muted-foreground text-sm">
            Showing {totalItems === 0 ? 0 : (localCurrentPage - 1) * localItemsPerPage + 1}-
            {Math.min(localCurrentPage * localItemsPerPage, totalItems)} of {totalItems} flights
          </div>
                    ...
                    <PaginationNext
                      ...
                      onClick={(e) => {
                        ...
                        if (localCurrentPage < totalPages) {
                          handlePageChange(Math.min(localCurrentPage + 1, totalPages))
                        }
                      }}
                      className="bg-background/90 border-2 border-primary/50 shadow-sm focus:border-primary"
                    />
